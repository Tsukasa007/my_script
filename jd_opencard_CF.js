...
!(async () => {
  ...
  $.shareUuid = 'ec142f1ba5f04fe9bd221b0450867ed9'
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      let newVar = await insertCrmPageVisit();
      $.actorUuid = await getActorUuid();
      $.log(newVar)
      await join(simpleActInfoVo.data.venderId)
      $.actorUuid = await getActorUuid();
      let newVar1 = await getDrawRecordHasCoupon();
      $.log(`${newVar1}`)
      $.log(`复制到浏览器进入活动页面 https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/5771276?activityId=dz2009100009970144&shareUuid=${$.shareUuid}`)
      if (i === 0 && $.actorUuid) {
        $.shareUuid = $.actorUuid;
      }else if (!$.actorUuid) {
...
function getDrawRecordHasCoupon() {
  return new Promise(resolve => {
    let body = `activityId=dz2009100009970144&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}`
    $.post(taskPostUrl('/dingzhi/taskact/common/getDrawRecordHasCoupon', body, 'https://lzdz-isv.isvjcloud.com/dingzhi/taskact/common/getDrawRecordHasCoupon'), async (err, resp, data) => {
      try {
        if (err) {
...
function insertCrmPageVisit() {
  return new Promise(resolve => {
    let body = `venderId=1000099701&elementId=去开卡&pageId=dz2009100009970144&pin=${encodeURIComponent($.myPingData.secretPin)}`
    $.post(taskPostUrl('/crm/pageVisit/insertCrmPageVisit', body,'https://lzdz-isv.isvjcloud.com/crm/pageVisit/insertCrmPageVisit'), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
  ...
  return new Promise(resolve => {
    $.post({
      ...
      headers: {
        ...
        'Referer':'https://lzdz-isv.isvjcloud.com/wxCommonInfo/token',
        'Cookie': cookie,
      }
    }, async (err, resp, data) => {
      try {
...
function getSimpleActInfoVo() {
  return new Promise(resolve => {
    $.post({
      ...
      body: 'activityId=dz2009100009970144',
      headers: {
...
function getHtml() {
  ...
  return new Promise(resolve => {
    $.get({
      url: `https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/5771276?activityId=dz2009100009970144&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
        'Host':'lzkj-isv.isvjcloud.com',
        'X-Requested-With': 'com.jingdong.app.mall',
        'Referer': `https://lzkj-isv.isvjcloud.com/pool/captain?activityId=dz2009100009970144&adsource=banner&lng=114.062592&lat=29.541468&sid=${$.shareUuid}&un_area=4_48201_54794_0`,
        'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE};`,
      }
    }, async (err, resp, data) => {
      try {
...
function adLog() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `venderId=1000004064&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=dz2009100009970144&pageUrl=https://lzdz-isv.isvjcloud.com/dingzhi/pg/phototGame/activity?activityId=dz2009100009970144&lng=114.062596&lat=29.541473&sid=${$.shareUuid}&un_area=4_48201_54794_0&subType=APP&adSource=`,
      headers: {
        'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
        'Host':'lzdz-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
...
function getActorUuid() {
  return new Promise(resolve => {
    $.post({
      url: `https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activityContent`,
      body: `activityId=dz2009100009970144&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
        'Host': 'lzdz-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
      ...
      try {
        if (err) {
          ...
          if (data.data.assistStatus === 3) {
            $.log("=====已助力过");
          }else if (data.data.assistStatus === 22) {
            $.log("=====老会员了");
          }else if(data.data.assistStatus === 1){
            $.log("=====助力成功");
          }else{
            $.log("=====助力状态未知");
          }
...
function share() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `activityId=dz2009100009970144&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
        'Host': 'lzdz-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
...
function ruhui(functionId) {
  return {
    url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=bindWithVender&body=%7B%22venderId%22%3A%22${functionId}%22%2C%22shopId%22%3A%22${functionId}%22%2C%22bindByVerifyCodeFlag%22%3A1%2C%22registerExtend%22%3A%7B%22v_sex%22%3A%22%E6%9C%AA%E7%9F%A5%22%2C%22v_birthday%22%3A%221996-11-11%22%2C%22v_name%22%3A%22y%E2%80%86y%E2%80%86y%22%7D%2C%22writeChildFlag%22%3A0%2C%22channel%22%3A801%7D&client=H5&clientVersion=9.2.0&uuid=88888&jsonp=jsonp_1629619563751_60421`,
    headers: {
      ...
      'User-Agent': 'Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1',
      'content-type': 'application/x-www-form-urlencoded',
      'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=1&channel=102&returnUrl=https%%3A%%2F%%2Flzdz-isv.isvjcloud.com%%2Fdingzhi%%2Fdz%%2FopenCard%%2Factivity%%2F7376465%%3FactivityId%%3Dd91d932b9a3d42b9ab77dd1d5e783839%%26shareUuid%%3Ded1873cb52384a6ab42671eb6aac841d`,
      'Cookie': cookie
    }
  }
}
